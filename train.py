'''
USAGE
python train.py --config_file=configs/ResNet_P.yaml --note=EfficientNet_P --device=4
'''
...
from utils import get_network, get_optimizer, write_json, initialize_wandb
...
def main(config_file, note, device=1):
    """
    Train math formula recognition model
    """
    options = Flags(config_file).get()

    # Start wandb (change plz)
    run = initialize_wandb('./configs/wandb.yaml', options, note)
    ...
    timestamp = datetime.strftime(kor_time, "%y%m%d-%H%M%S")
    log_path = os.path.join(options.prefix, timestamp)
    ...
    shutil.copy(config_file, os.path.join(log_path, "train_config_yaml"))

    write_json(os.path.join(log_path, "args.json"), options)

    if options.print_epochs is None:
        options.print_epochs = options.num_epochs
        
    # writer = init_tensorboard(name=options.prefix.strip("-"))
    writer = init_tensorboard(name=log_path.strip("-"))
    ...
    run.summary['best epoch valid accuracy'] = valid_accuracy[run.summary['best_epoch']]
    run.summary['directory'] = timestamp
